...
### Copyright (C) 2013-2014 by Jim Klimov
...
CURRENT_RPOOL="`grep -w / /etc/mnttab | grep -w zfs | sed 's,^\([^\/]*\)/.*,\1,'`" || CURRENT_RPOOL=""

[ x"$BEOLD" = x ] && BEOLD="$CURRENT_BE"
[ x"$BENEW" = x ] && BENEW="`echo "$BEOLD" | sed 's/^\([^\-]*\)-.*$/\1/'`-`date -u "+%Y%m%dZ%H%M%S"`"
...
[ x"$RPOOLALT" = x ] && RPOOLALT="`zpool get -Hp altroot "$RPOOL" | awk '{print $3}'`"
...
[ x"$EXCLUDE_ATTRS" = x ] && EXCLUDE_ATTRS='org.opensolaris.libbe:uuid|canmount|mountpoint'

echo "=== Will clone $BEOLD into $BENEW, ok?"
